package scot.oskar.permissible.command.user;
...
public class UserPermissionAddCommand extends AbstractWorldCommand {
  ...
  public UserPermissionAddCommand() {
    ...
    this.requirePermission("permissible.user.permission.add");
  }
  ...
  protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
    ...
    if (oldAttachment.getPermissions().contains(permission)) {
      context.sendMessage(Message.raw("Player already has permission: " + permission).color(Color.RED));
      return;
    }
    ...
    context.sendMessage(Message.raw("Added permission '" + permission + "' to player").color(Color.GREEN));
  }
}
