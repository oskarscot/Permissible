...
public class GroupAddCommand extends AbstractWorldCommand {

  private final RequiredArg<UUID> playerArg;
  private final RequiredArg<String> groupArg;

  public GroupAddCommand() {
    super("add", "Adds a group to a user.");
    this.playerArg = this.withRequiredArg("player", "The player to add the group to", ArgTypes.PLAYER_UUID);
    this.groupArg = this.withRequiredArg("group", "The group name",  ArgTypes.STRING);
    this.requirePermission("permissible.user.add.group");
  }
  ...
  protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
    ...
    if (oldAttachment.getGroups().contains(group)) {
      context.sendMessage(Message.raw("Player is already in group: " + group).color(Color.RED));
      return;
    }
    ...
    context.sendMessage(Message.raw("Added player to group: " + group).color(Color.GREEN));
  }
}
